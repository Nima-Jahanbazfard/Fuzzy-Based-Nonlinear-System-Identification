...
% define a g(x) function and calculating norm-inf of d2g(x) in [x_min x_max] 
g =  (10*(x^4)*((exp(x)+exp(-1*x))/2));
d2g = diff(g,x,2);
d2g_func= matlabFunction(d2g);
x_vals=linspace(x_min,x_max,1000) ;
d2g_vals=abs(d2g_func(x_vals));
norm_inf=max(d2g_vals);

%plot of g(x) and g2d(x)
ezplot(g,[x_min x_max])
title('g(x)')
figure;
ezplot(d2g,[x_min x_max])
title('d2g(x)')
figure;
